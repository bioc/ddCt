...
Sweave("rt\_PCR\_Skript\_ddCt\_X\_X.Rnw")

\noindent The X might differ, depending on the version of this script. In the
current example the script is  version '3\_0':

Sweave("rt\_PCR\_Skript\_ddCt\_3\_0.Rnw")
...
dev.print(pdf, getFileName("errbarplot_bySample_", sho), width=15, height=15)
result.byDetector <- errBarchart(result, by="Detector")
print(result.byDetector)
dev.print(pdf, getFileName("errbarplot_byDetector_", sho), width=15, height=15)
@
\newpage
